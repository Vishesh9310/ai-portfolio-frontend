...
const FullProject = () => {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        ...
        const datadata = await fetchDataProjects();
        setDataProjects(Array.isArray(datadata) ? datadata : []);

        const averagedata = await fetchAverageProjects();
        setAverageProjects(Array.isArray(averagedata) ? averagedata : []);
      } catch (err) {
  ...
  const project =
    topprojects.find((p) => p.id === Number(id)) ||
    dataprojects.find((p) => p.id === Number(id)) ||
    averageprojects.find((p) => p.id === Number(id));
    ...
    <div className="project-body h-fit">
      <div className="pt-24 px-4 max-w-6xl mx-auto space-y-8 pb-10">
        ...
        <div>
          ...
          <p className="text-gray-300">Tech Stack: {project.tech?.join(", ")}</p>
        </div>
        ...
        <div className="text-center space-y-4">
          <h1 className="text-2xl sm:text-4xl font-bold text-purple-600">{project.title}</h1>
          ...
          <div className="leading-relaxed space-y-2 text-left">
            {Array.isArray(project.about)
              ? project.about.map((line, index) => <p key={index}>{line}</p>)
              : <p>{project.about}</p>}
          </div>
        </div>
